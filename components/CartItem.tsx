...
const CartItemCard: React.FC<CartItemProps> = ({ item }) => {
  ...
  const handleIncrease = () => {
    // Prevent increasing beyond the product's stock (as recorded in the cart item)
    if (currentQuantity < item.product.stock) {
      updateQuantity(item.product.id, currentQuantity + 1);
      startCollapseTimer();
    } else {
      // Optionally, inform user they've reached stock limit
      console.warn(`Cannot increase quantity for ${item.product.name} beyond available stock of ${item.product.stock}.`);
      startCollapseTimer(); // Still reset timer to collapse eventually
    }
  };
  ...
  // If for some reason the item is no longer in cart (currentQuantity is 0), don't render anything
      ...
      <div className="flex flex-col items-end ml-2 space-y-1 flex-shrink-0"> {/* Reduced ml-4 to ml-2 for space */}
        {isExpanded ? (
          <div className="flex items-center bg-white rounded-full shadow-lg border border-gray-300 overflow-hidden">
            ...
            >
              <Icon name={currentQuantity === 1 ? "trash" : "minus"} size={16} /> {/* Smaller icon, show trash if qty is 1 */}
            </button>
            ...
            <button
              ...
              className="p-1.5 sm:p-2 text-panda-pink hover:bg-pink-50 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
              aria-label={`Increase quantity of ${item.product.name}`}
              disabled={currentQuantity >= item.product.stock} // Disable if at stock limit
            >
